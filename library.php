...
class Library 
{
    ...
    {
        $data = $this->db->prepare('INSERT INTO tb_mahasiswa1 (nama_mahasiswa,kelas,alamat) VALUES (?, ?, ?)');
    ...
    public function update($kd_mahasiswa,$nama_mahasiswa,$kelas,$alamat){
        $query = $this->db->prepare('UPDATE tb_mahasiswa1 set nama_mahasiswa=?,kelas=?,alamat=? where kd_mahasiswa=?');
